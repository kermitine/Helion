...
GRIPPER_PWM_HZ = 50.0
GRIPPER_DEFAULT_PULSE_MIN_US = 500.0
GRIPPER_DEFAULT_PULSE_MAX_US = 2500.0
...
GRIPPER_DEFAULT_RELEASE_AFTER_MOVE = False
GRIPPER_SETTLE_S = 0.45
GRIPPER_RAMP_SPEED_DEG_S = 85.0
GRIPPER_RAMP_INTERVAL_S = 0.020
...
GRIPPER_ADAPTIVE_GRIP_MAX_POSITION = 0.20
GRIPPER_ADAPTIVE_GRIP_MAX_RELAX_DEG = 30.0
GRIPPER_ADAPTIVE_GRIP_MAX_SQUEEZE_S = 2.0
GRIPPER_ADAPTIVE_GRIP_RELAX_SETTLE_S = 0.12
ARM_PLANNER_MAX_BLOCKS = 48
ARM_PLANNER_MAX_EXPANDED_BLOCKS = 240
ARM_PLANNER_MAX_ROUTE_WAYPOINTS = 6000
ARM_PLANNER_DEFAULT_WAIT_S = 0.50
ARM_PLANNER_MIN_WAIT_S = 0.0
...
class DashboardController:
    def __init__(
        ...
        self.arm_plan_blocks = self.default_arm_plan_blocks()
        ...
        self.gripper_release_after_move = GRIPPER_DEFAULT_RELEASE_AFTER_MOVE
        self.gripper_adaptive_grip_enabled = GRIPPER_ADAPTIVE_GRIP_DEFAULT_ENABLED
        self.gripper_adaptive_grip_relax_deg = GRIPPER_ADAPTIVE_GRIP_DEFAULT_RELAX_DEG
        self.gripper_adaptive_grip_squeeze_s = GRIPPER_ADAPTIVE_GRIP_DEFAULT_SQUEEZE_S
    ...
    def apply_gripper_payload(self, payload: Dict[str, Any]) -> None:
        ...
        with self.lock:
            ...
            self.gripper_adaptive_grip_enabled = parse_bool(
                value("gripperAdaptiveGrip", value("adaptiveGrip", self.gripper_adaptive_grip_enabled)),
                self.gripper_adaptive_grip_enabled,
            )
            self.gripper_adaptive_grip_relax_deg = clamped_float(
                value("gripperGripRelaxDeg", value("gripRelaxDeg", self.gripper_adaptive_grip_relax_deg)),
                self.gripper_adaptive_grip_relax_deg,
                0.0,
                GRIPPER_ADAPTIVE_GRIP_MAX_RELAX_DEG,
            )
            self.gripper_adaptive_grip_squeeze_s = clamped_float(
                value("gripperGripSqueezeS", value("gripSqueezeS", self.gripper_adaptive_grip_squeeze_s)),
                self.gripper_adaptive_grip_squeeze_s,
                0.0,
    ...
    def default_arm_plan_blocks(self) -> List[Dict[str, Any]]:
        return [
            ...
            },
            {
                "type": "wait",
                "name": "Pause",
                "seconds": ARM_PLANNER_DEFAULT_WAIT_S,
            },
        ]

    def arm_plan_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        plan = payload.get("armPlan")
    ...
    def sanitize_arm_plan_block(self, block: Any, index: int) -> Dict[str, Any]:
        if not isinstance(block, dict):
            block = {}
        block_type = str(block.get("type") or "move").strip().lower()
        if block_type in ("position", "pose", "target"):
            block_type = "move"
        elif block_type in ("gap", "pause", "delay", "hold"):
            block_type = "wait"
        elif block_type in ("repeat",):
            block_type = "loop"
        elif block_type not in ("move", "wait", "loop"):
            block_type = "move"

        name = str(block.get("name") or block.get("label") or "").strip()[:48]
        if block_type == "move":
            target = block.get("target")
            if not isinstance(target, dict):
                target = block
            fallback = self.arm_target
            return {
                "type": "move",
                "name": name or f"Position {index + 1}",
                "target": {
                    "x": parse_float(target.get("x"), fallback["x"]),
        ...
        if block_type == "wait":
            return {
                "type": "wait",
                "name": name or f"Gap {index + 1}",
                "seconds": clamped_float(
                    block.get("seconds", block.get("duration", block.get("time", ARM_PLANNER_DEFAULT_WAIT_S))),
                    ARM_PLANNER_DEFAULT_WAIT_S,
                    ARM_PLANNER_MIN_WAIT_S,
    ...
    def sanitize_arm_plan_blocks(self, payload: Dict[str, Any]) -> List[Dict[str, Any]]:
        plan = self.arm_plan_payload(payload)
        raw_blocks = plan.get("blocks", payload.get("armPlanBlocks"))
        if not isinstance(raw_blocks, list):
            raw_blocks = self.arm_plan_blocks
        blocks = [
            self.sanitize_arm_plan_block(block, index)
            for index, block in enumerate(raw_blocks[:ARM_PLANNER_MAX_BLOCKS])
        ]
        if not blocks:
            blocks = self.default_arm_plan_blocks()
        return blocks

    def apply_arm_plan_payload(self, payload: Dict[str, Any]) -> None:
    ...
    def expand_arm_plan_blocks(
        ...
        def append_expanded(block: Dict[str, Any]) -> None:
            if len(expanded) >= ARM_PLANNER_MAX_EXPANDED_BLOCKS:
                raise ValueError(f"Arm planner expands beyond {ARM_PLANNER_MAX_EXPANDED_BLOCKS} blocks")
        ...
        def expand_index(index: int, loop_stack: Tuple[int, ...]) -> None:
            ...
            if block_type in ("move", "wait"):
                append_expanded(block)
                return
            if block_type != "loop":
    ...
    def gripper_ramp_angles(self, start_angle_deg: float, target_angle_deg: float) -> List[float]:
        ...
        distance = abs(delta)
        if distance <= GRIPPER_ANGLE_DEADBAND_DEG:
            return [target_angle]
        duration_s = max(
            GRIPPER_RAMP_MIN_DURATION_S,
            min(GRIPPER_RAMP_MAX_DURATION_S, distance / GRIPPER_RAMP_SPEED_DEG_S),
        )
        steps = max(1, int(math.ceil(duration_s / GRIPPER_RAMP_INTERVAL_S)))
        angles = []
        for index in range(1, steps + 1):
            t = index / steps
            eased = 0.5 - (0.5 * math.cos(math.pi * t))
            angles.append(start_angle + (delta * eased))
        return angles

    def gripper_angle_moves_toward_open(self, from_angle_deg: float, target_angle_deg: float) -> bool:
        with self.lock:
            open_angle = self.gripper_open_angle_deg
            closed_angle = self.gripper_closed_angle_deg
        open_direction = open_angle - closed_angle
        if abs(open_direction) <= GRIPPER_ANGLE_DEADBAND_DEG:
            return False
        return ((target_angle_deg - from_angle_deg) * open_direction) > 0.0

    def gripper_relaxed_grip_angle(self, grip_angle_deg: float) -> float:
        with self.lock:
            open_angle = self.gripper_open_angle_deg
            closed_angle = self.gripper_closed_angle_deg
            relax_deg = self.gripper_adaptive_grip_relax_deg
        open_direction = open_angle - closed_angle
        if abs(open_direction) <= GRIPPER_ANGLE_DEADBAND_DEG or relax_deg <= 0.0:
            return max(0.0, min(180.0, float(grip_angle_deg)))
        relaxed = grip_angle_deg + ((1.0 if open_direction > 0.0 else -1.0) * relax_deg)
        low_angle = min(open_angle, closed_angle)
        high_angle = max(open_angle, closed_angle)
        return max(low_angle, min(high_angle, max(0.0, min(180.0, relaxed))))
    ...
    def move_gripper_angle(
        ...
        adaptive_grip = bool(adaptive_grip)
        try:
            with self.lock:
                ...
                quiet_grip = self.gripper_quiet_grip
                last_grip_angle = self.gripper_last_grip_angle_deg
                squeeze_s = self.gripper_adaptive_grip_squeeze_s
            start_angle = float(start_angle)
            if (
                adaptive_grip
                and quiet_grip
                and last_grip_angle is not None
                and abs(angle - float(last_grip_angle)) <= GRIPPER_ANGLE_DEADBAND_DEG
            ):
                return {
                    "ok": True,
                    "message": f"Gripper already quiet at {angle:.1f} deg",
                    "angleDeg": angle,
                    "relaxedAngleDeg": start_angle,
                    "attached": False,
                    "adaptiveGrip": True,
                    "quietGrip": True,
                }
            pwm = self.ensure_gripper_pwm()
            if quiet_grip and self.gripper_angle_moves_toward_open(start_angle, angle):
                ...
                and abs(angle - start_angle) <= GRIPPER_ANGLE_DEADBAND_DEG
            ):
                pass
            else:
                self.drive_gripper_ramp(pwm, start_angle, angle)
            final_angle = angle
            if adaptive_grip:
                if squeeze_s > 0.0:
                    time.sleep(squeeze_s)
                if release_after_move:
                    relaxed_angle = self.gripper_relaxed_grip_angle(angle)
                    if abs(relaxed_angle - angle) > GRIPPER_ANGLE_DEADBAND_DEG:
                        self.drive_gripper_ramp(pwm, angle, relaxed_angle)
                        final_angle = relaxed_angle
                        time.sleep(GRIPPER_ADAPTIVE_GRIP_RELAX_SETTLE_S)
                    self.release_gripper(log=False)
                    with self.lock:
                        self.gripper_test_angle_deg = angle
                        self.gripper_last_angle_deg = final_angle
                        self.gripper_attached = False
                        self.gripper_quiet_grip = True
                        self.gripper_last_grip_angle_deg = angle
                        self.gripper_last_error = ""
                    if not quiet:
                        self.log(
                    ...
                    return {
                        "ok": True,
                        "message": (
                            f"Gripper adaptive grip {angle:.1f} deg, "
    ...
    def move_gripper_position(
        self,
        position: float,
        quiet: bool = False,
        adaptive_grip: Optional[bool] = None,
    ) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            self.gripper_test_angle_deg = angle
            adaptive_enabled = self.gripper_adaptive_grip_enabled if adaptive_grip is None else adaptive_grip
        use_adaptive_grip = bool(adaptive_enabled) and percent <= GRIPPER_ADAPTIVE_GRIP_MAX_POSITION
        result = self.move_gripper_angle(
            angle,
            quiet=quiet,
            adaptive_grip=use_adaptive_grip,
        )
        if result.get("ok"):
            result["position"] = percent
            if result.get("adaptiveGrip"):
                suffix = " adaptive grip"
            else:
    ...
    def run_gripper_command(self, command: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if command == "gripper-config":
            self.log(
                ...
                f"adaptive={'on' if self.gripper_adaptive_grip_enabled else 'off'}"
            )
        ...
        if not self.command_lock.acquire(blocking=False):
            if command in (
                ...
                try:
                    ...
                    elif command == "arm-plan-stop":
                        return self.stop_arm_plan()
    ...
    def _run_command(self, command: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if command == "arm-plan-start":
            return self.start_arm_plan(payload)
    ...
    def apply_values_payload(self, payload: Dict[str, Any]) -> Tuple[bool, str, int, bool]:
        ...
        with self.lock:
            ...
            self.apply_arm_plan_payload(payload)
            bus_changed = (
                new_serial_port != old_serial_port
                or new_serial_baud != old_serial_baud
            )
    ...
    def stop_arm_plan_state(self) -> bool:
        with self.lock:
            was_active = self.arm_plan_active
            self.arm_plan_active = False
            self.arm_plan_started_at = 0.0
            self.arm_plan_duration_s = 0.0
            self.arm_plan_forever_active = False
    ...
    def arm_route_waypoint_interval(self, waypoint: Dict[str, Any]) -> float:
        ...
        return max(0.005, min(ARM_PLANNER_MAX_WAIT_S, interval))
    ...
    def build_arm_plan_route(
        ...
        for block in expanded_blocks:
            ...
            if block_type == "move":
                target = block.get("target")
                if not isinstance(target, dict):
                    raise ValueError("Move block is missing a target")
                segment, final_target, final_joint_angles = self.arm_route_waypoints_to_target(
                    target,
                    previous_angles,
                    ARM_ROUTE_MAX_STEP_RAD,
                ...
                if seconds > 0.0:
                    route_waypoints.append(self.arm_hold_waypoint(previous_angles, seconds))
                    wait_count += 1
            else:
                raise ValueError(f"Expanded planner included unexpected block type '{block_type}'")
    ...
    def start_arm_plan(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ok, message = self.validate_arm_command_motors(payload)
        if not ok:
            return {"ok": False, "message": message}
        self.stop_arm_plan_state()
        ...
        route_ok = self.start_arm_route(
            route_waypoints,
            config_signature,
            f"Arm planner ({self.arm_joint_count}-joint)",
        )
        if not route_ok:
            return {"ok": False, "message": "Arm planner failed to start"}
        now = time.monotonic()
        with self.lock:
    ...
    def stop_arm_plan(self) -> Dict[str, Any]:
        ...
        if self.connected and self.arm_position_configured:
            for axis in self.active_arm_axes():
                self.send_arm_operation_control_for_axis(axis)
            self.last_arm_position_refresh_at = time.monotonic()
        message = "Arm planner stopped; holding current pose" if was_active else "Arm planner already idle"
        self.log(message)
        return {"ok": True, "message": message}
    ...
    def active_command_motor_ids(self) -> List[int]:
        ...
        if self.arm_position_configured or self.arm_route_waypoints or self.arm_plan_active:
